...
consolidated `section_5_2.txt` and `section_5_3.txt` files, splits them into
operation blocks, and writes a compact JSON artifact containing the raw text
for each operation.
"""
...
SPEC_DIR = ROOT_DIR / "data" / "specs" / "segments"
OUTPUT_FILE = ROOT_DIR / "data" / "generated" / "operation_descriptions.json"
...
def save_operation_descriptions(operations: List[OperationDescription], output_file: Path) -> None:
    output_file.parent.mkdir(parents=True, exist_ok=True)
    output_file.write_text(
        json.dumps([operation.to_dict() for operation in operations], indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    save_operation_descriptions(operations, OUTPUT_FILE)
    print(f"[✓] Saved operation descriptions to {OUTPUT_FILE}", flush=True)
